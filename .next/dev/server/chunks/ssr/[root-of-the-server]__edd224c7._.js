...
function ChoicePricingPage() {
    ...
    const initUser = async ()=>{
        ...
        if (savedUserId && !savedUserId.startsWith('user-')) {
            ...
            // Optional: You can create the user here immediately if you want
            // but do not use "user-date". Use a real Supabase insert if needed.
            setUserId(null);
        }
    };
    ...
    const handleConciergeSelection = ()=>{
        router.push('/concierge-confirmation');
    };
    const handleFreeStart = (mode)=>{
        if (!userId) {
            alert('Setting up your account...');
            return;
        }
        // 1. Save the mode
        localStorage.setItem('legacy-vault-mode', mode);
        // 2. IMPORTANT: Go directly to the dashboard, NOT confirmation
        if (mode === 'personal') {
            router.push(`/dashboard/personal/${userId}`);
        } else {
            router.push(`/dashboard/family/${userId}`);
        }
    };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__edd224c7._.js.map