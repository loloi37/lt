...
export default function FamilyLinker({ currentMemorialId, userId }: FamilyLinkerProps) {
    ...
    const loadData = async () => {
        ...
        // Fetch candidates (other memorials owned by user)
        const { data: others } = await supabase
            .from('memorials')
            .select('id, full_name')
            .eq('user_id', userId)
            .eq('deleted', false)
            .neq('id', currentMemorialId);
    ...
    if (loading) {
        return (
            <div className="p-4 text-center">
                <Loader2 className="animate-spin inline text-mist" />
            </div>
        );
    }
    ...
    return (
        <div className="bg-sand/10 rounded-xl p-6 border border-sand/30">
            ...
            {relations.length > 0 ? (
                ...
                <p className="text-sm text-charcoal/50 italic mb-6">No connections yet. Link this memorial to others in your family archive.</p>
            )}

            {/* Add New Link Form */}
            {candidates.length > 0 ? (
                <div className="flex flex-col gap-3">
                    <p className="text-xs font-medium text-charcoal/70 uppercase tracking-wider font-sans">Add Connection</p>
                    <div className="flex gap-2">
                        <select
                            className="flex-1 p-2 rounded-lg border border-sand/30 text-sm focus:outline-none focus:border-mist"
                        ...
                        <span className="self-center text-charcoal/40 font-serif italic text-sm">is</span>

                        <select
                            className="w-32 p-2 rounded-lg border border-sand/30 text-sm focus:outline-none focus:border-mist"
                            value={selectedType}
                    ...
                    <button
                        ...
                        className="w-full py-2 bg-charcoal text-ivory rounded-lg text-sm font-medium font-sans hover:bg-charcoal/90 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                    >
