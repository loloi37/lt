...
export default function Step9Videos({ data, onUpdate, onNext, onBack, memorialId }: Step9Props) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        newVideos.push({
          ...
          thumbnail: thumbnailUrl || videoPublic.publicUrl, // Fallback to video URL
  ...
  const updateVideoTitle = (id: string, title: string) => {
    onUpdate({
      videos: data.videos.map(v => (v.id === id ? { ...v, title } : v))
    });
  };
