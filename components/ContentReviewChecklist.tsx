    ...
    return (
        <div className="dark-card p-6">
            <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-sm font-semibold text-vault-text font-sans">Content Review</h3>
            </div>
            <p className="text-xs text-vault-muted font-sans mb-5">
                ...
                {CHECKLIST_ITEMS.map(item => (
                    ...
                    >
                        {checked.has(item.id) ? (
                            ...
                            <Square size={18} className="text-vault-muted flex-shrink-0 mt-0.5" />
                        )}
                        <div>
                            <p className="text-sm font-sans font-medium text-vault-text">{item.label}</p>
                            <p className="text-xs text-vault-muted font-sans mt-0.5">{item.description}</p>
                        </div>
                    </button>
                ))}
            </div>
            ...
            <div className="flex gap-3">
                ...
                <button
                    onClick={onEditFirst}
                    className="px-4 py-2.5 text-sm font-sans font-medium text-vault-muted hover:text-vault-text transition-colors"
                >
                    Edit First
                </button>
            </div>
